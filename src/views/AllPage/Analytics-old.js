...
const styles = {
  ...
  nav:        { width:210, minWidth:180, background:"#fff", borderRight:"1px solid #e8eaf0", padding:"24px 0", display:"flex", flexDirection:"column", gap:4, borderRadius:"0 16px 16px 0" },
  ...
  content:    { flex:1, padding:"28px 24px", overflowX:"hidden" },
  ...
  filterPanel:{ width:220, minWidth:200, background:"#f7f8fc", borderRadius:12, padding:16, border:"1px solid #e8eaf0", flexShrink:0 },
...
function TagSearch({ label, all, selected, onToggle, searchPlaceholder }) {
  ...
  return (
    <div style={{marginBottom:14}}>
      ...
      <input style={styles.input} placeholder={searchPlaceholder||`Search ${label||""}…`} value={q} onChange={e=>setQ(e.target.value)} />
      <div style={{maxHeight:140, overflowY:"auto", marginTop:6, display:"flex", flexDirection:"column", gap:2}}>
        {filtered.map(item=>(
          <label key={item} style={{display:"flex", alignItems:"center", gap:6, padding:"4px 2px", cursor:"pointer", fontSize:12, color:"#374151"}}>
      ...
      {selected.length>0 && (
        <div style={{marginTop:6, display:"flex", flexWrap:"wrap"}}>
          {selected.map(s=>(
            <span key={s} style={styles.chip(true)}>
              {s}
              <span role="button" tabIndex={0} onClick={()=>onToggle(s)} onKeyDown={e=>{if(e.key==="Enter"||e.key===" ")onToggle(s);}} style={{cursor:"pointer", opacity:.7, fontSize:10}}>✕</span>
            </span>
          ))}
        </div>
      )}
...
function AgeRangeFilter({ value, onChange }) {
  return (
    <div style={{marginBottom:14}}>
      ...
      {["All",...Object.keys(AGE_GROUPS)].map(g=>(
        <label key={g} style={{display:"flex",alignItems:"center",gap:6,padding:"3px 0",cursor:"pointer",fontSize:12,color:value===g?ACCENT:"#374151"}}>
          <input type="radio" checked={value===g} onChange={()=>onChange(g)} style={{accentColor:ACCENT}} />
          {g}
        </label>
      ))}
    </div>
