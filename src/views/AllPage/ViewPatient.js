...
function ViewPatient() {
  ...
  const handleAddNote = async () => {
    ...
    try {
      const response = await axios.post(`${Base_Url}add_note`, {
        user_id: user_id,
  ...
  const handleDeleteNote = async (noteId) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await axios.post(`${Base_Url}delete_note`, {
          note_id: noteId
        }, {
          headers: {
            Authorization: `Bearer ${token}`
  ...
  const filterMedicationData = medication?.filter((item) => {
  ...
  const filterAdverseData = adverse_data?.filter((item) => {
    const lowercasedTerm = searchQueryGroup.toLowerCase();
    return Object.values(item).some(
      val => val && String(val).toLowerCase().includes(lowercasedTerm)
    )
  ...
  const filterLabReportsData = report?.filter((item) => {
    const lowercasedTerm = searchQueryGroup.toLowerCase();
    return Object.values(item).some(
      val => val && String(val).toLowerCase().includes(lowercasedTerm)
    )
  ...
  const filterMeasurementData = measurement?.filter((item) => {
    const lowercasedTerm = searchQueryGroup.toLowerCase();
    return Object.values(item).some(
      val => val && String(val).toLowerCase().includes(lowercasedTerm)
    )
  ...
  const filterNotesData = notes?.filter((item) => {
    const lowercasedTerm = searchQueryGroup.toLowerCase();
    return Object.values(item).some(
      val => val && String(val).toLowerCase().includes(lowercasedTerm)
    )
  ...
  const filterComplianceData = complianceData?.filter((item) => {
    const lowercasedTerm = searchQueryGroup.toLowerCase();
    return Object.values(item).some(
      val => val && String(val).toLowerCase().includes(lowercasedTerm)
    )
        ...
        {data.length > 0 ? (
                ...
                <thead>
                  <tr>
                    {columns.map((col, index) => (
                      <th key={index} style={{ textAlign: 'center', fontWeight: '500', whiteSpace: 'wrap' }}>{col.header}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={index}>
                      ...
                      <td key={colIndex} style={{ textAlign: 'center', whiteSpace: 'wrap' }}>
                        {col.key === 'taken_status'
                          ? (item.taken_status ? 'Taken' : 'Not Taken')
          ...
          <Table responsive hover>
            <thead>
              <tr>
                {columns.map((col, index) => (
                  <th key={index} style={{ textAlign: 'center', fontWeight: '500' }}>{col.header}</th>
                ))}
              </tr>
            </thead>
            <tbody>
        ...
        return (
          <>
            ...
            {filterLabReportsData.length > 0 && (
              <div className="d-flex justify-content-between">
                <p style={{ fontWeight: '500' }} className="pagination">
                  Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, filterLabReportsData.length)} of {filterLabReportsData.length} entries
                </p>
                <Stack spacing={2} alignItems="right">
          ...
          <>
            {measurement.length > 0 && (
              <div className="mb-3 text-end">
                <Button
                  variant="success"
                  onClick={exportMeasurementToExcel}
                  style={{ backgroundColor: '#1DDEC4', border: 'none' }}
                >
                  Export to Excel
                </Button>
      ...
      {loading ? (
        ...
        <>
          <Typography style={{ marginTop: '15px', marginBottom: '30px' }} variant="h4" gutterBottom>
            <span style={{ color: '#238BF0' }}>Dashboard</span> / View Patient
          </Typography>
          <Card className="mb-5">
            <Card.Body>
                    ...
                    <div className="user-detail row " style={{ marginTop: '20px' }}>
                      <div className="col-lg-12">
                        ...
                        <div className="row">
                          ...
                          <div className="col-lg-8">
                            <p style={{ fontWeight: '500', marginLeft: '50px;' }}>{user_data?.email || '-'}</p>
                          </div>
                        </div>
                        ...
                        <div className="row">
                          ...
                          <div className="col-lg-8 responsive-btn">
                            <p
                              style={{
                                borderRadius: '25px',
                                backgroundColor: user_data.active_flag === 1 ? '#009640' : '#FF2222',
                                padding: '0px 15px',
                                width: '109px',
                                color: '#fff',
                                fontWeight: '500',
          ...
          <nav className="col-xl-10 navbar navbar-expand-lg navbar-light">
            <div className="container-fluid tabs justify-content-start" style={{ marginBottom: '41px' }}>
              <button
                ...
                className={`btn ${content === contentTypes.note ? 'btn-primary' : 'btn-light'}`}
                type="button"
              ...
              <button
                style={{ border: '1px solid #238BF0', borderRadius: '0px', height: '38px' }}
                className={`btn ${content === contentTypes.medication ? 'btn-primary' : 'btn-light'}`}
                type="button"
              ...
              <button
                style={{ border: '1px solid #238BF0', borderRadius: '0px', height: '38px' }}
                className={`btn ${content === contentTypes.labReports ? 'btn-primary' : 'btn-light'}`}
                type="button"
              ...
              <button
                style={{ border: '1px solid #238BF0', borderRadius: '0px', height: '38px' }}
                className={`btn ${content === contentTypes.measurement ? 'btn-primary' : 'btn-light'}`}
                type="button"
              ...
              <button
                style={{ border: '1px solid #238BF0', borderRadius: '0px', height: '38px' }}
                className={`btn ${content === contentTypes.adverse ? 'btn-primary' : 'btn-light'}`}
                type="button"
          ...
          <Modal show={editingNoteId !== null} onHide={() => {
            setEditingNoteId(null);
            setEditDescriptionError('');
          }} centered>
            <Modal.Header>
